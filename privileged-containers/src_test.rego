...
test_input_container_not_previleged_allowed { 
    input := { "review": input_review} 
    results := violation with input as input
    count(results) == 0
}
test_input_container_previleged_not_allowed { 
    input := { "review": input_review_priv} 
    results := violation with input as input
    count(results) > 0
}
test_input_container_many_not_previleged_allowed { 
    input := { "review": input_review_many} 
    results := violation with input as input
    count(results) == 0
}
test_input_container_many_mixed_previleged_not_allowed { 
    input := { "review": input_review_many_mixed} 
    results := violation with input as input
    count(results) > 0
}
test_input_container_many_mixed_previleged_not_allowed_two { 
